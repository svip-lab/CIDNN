...
class Trace2Trace_LN:
    ...
    def train(self, train_input_traces, train_target_traces, fixed_net):
    ...
    def main(self, model_name, log, fixed_net, pre_train_model_path):
        cur_time = time.strftime('%Y-%m-%d-%X', time.localtime())
        self.pre_train_model_path = pre_train_model_path
        self.model_path = 'model/%s/%s' % (model_name, cur_time)
        self.init_net(fixed_net)
        self.vis = visdom.Visdom(env=model_name + 'fixed net' + fixed_net)
        ...
        if log:
            ...
            self.log_file.write('fixed_net: %d\n' % fixed_net)
        ...
        for epoch in xrange(1, self.n_epochs + 1):
            for i, (train_input_traces, train_target_traces) in enumerate(self.train_loader):
                ...
                L2_square_loss, MSE_loss = self.train(train_input_traces, train_target_traces, fixed_net)
                ...
                self.vis.line(np.array(self.train_loss_list), win='train_loss', opts={'title': 'train_loss'})
            ...
            self.vis.line(np.array(self.test_loss_list), win='test_loss', opts={'title': 'test_loss'})
...
def main():
    ...
    model.main('new-cross-domain-TSD-GC', log=True, fixed_net=2, pre_train_model_path='model/Cross-domain-OC-TSD/lr2e-3_cross_domain/lr_0.002_iter_40900')
