    ...
    def main(self, model_name, log):
        ...
        for epoch in xrange(1, self.n_epochs + 1):
            for i, (train_input_traces, train_target_traces) in enumerate(self.train_loader):
                ...
                self.vis.line(Y=np.array(self.train_loss_list), X=np.arange(train_count), win='train', opts={'title': 'train loss'})
            ...
            self.vis.line(Y=np.array(self.test_loss_list), X=np.arange(epoch), win='test', opts={'title': 'test loss'})
            print test_loss_msg
            if log:
...
def main():
    ...
    model.load_data('data/GC/noise_%d_xy_dataset.npz' % noise_rate)
    model.n_layers = 2
    model.vis = visdom.Visdom(env='noise-GC-%d' % noise_rate)
    model.main('noise-GC-%d' % noise_rate, log=True)
